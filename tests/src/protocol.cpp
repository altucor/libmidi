...
TEST(protocol, midi_cmd_bit_fields)
{
    ...
    EXPECT_EQ(cmd.system, MIDI_STATUS_SYSTEM_COMMON_SONG_POSITION);
}
...
TEST(protocol, midi_status)
{
    ...
    EXPECT_EQ(MIDI_STATUS_COUNT, 8);
}

TEST(protocol, midi_status_system)
{
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_SYSEX_START, 0);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_MTC_QUARTER_FRAME, 1);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_SONG_POSITION, 2);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_SONG_SELECT, 3);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_RESERVED_4, 4);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_UNOFFICIAL_BUS_SELECT, 5);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_TUNE_REQUEST, 6);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_SYSEX_END, 7);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_TIMING_TICK, 8);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_RESERVED_9, 9);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_SONG_START, 10);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_SONG_CONTINUE, 11);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_SONG_STOP, 12);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_RESERVED_13, 13);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_ACTIVE_SENSING, 14);

    EXPECT_EQ(MIDI_STATUS_SYSTEM_COMMON_META, 15);
    EXPECT_EQ(MIDI_STATUS_SYSTEM_REALTIME_RESET, 15);

    EXPECT_EQ(MIDI_STATUS_SYSTEM_COUNT, 16);
}
...
TEST(protocol, midi_meta_event)
{
    ...
    EXPECT_EQ(MIDI_META_EVENT_MIDI_CHANNEL_PREFIX, 0x20);
