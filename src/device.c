...
{
    midi_device_t *ctx = (midi_device_t *)calloc(1, sizeof(midi_device_t));
    ctx->channel = channel;
...
void midi_device_process_new_status_system(midi_device_t *midi, uint8_t b)
{
    switch (b)
    {
...
void midi_device_process_new_status(midi_device_t *ctx, uint8_t b)
{
    ...
    {
        midi_device_process_new_status_system(ctx, channel);
        return;
    }
...
void midi_device_synth_send(midi_device_t *ctx, midi_device_callback_data_t *synth)
{
...
void midi_device_process_payload(midi_device_t *ctx)
{
    if (ctx->synth_cb[MIDI_CHANNEL_ANY] != NULL)
    {
        midi_device_synth_send(ctx, ctx->synth_cb[MIDI_CHANNEL_ANY]);
    }
    for (uint8_t i = 0; i < 16; i++)
    {
        if (ctx->synth_cb[i] == NULL)
            continue;
        midi_device_synth_send(ctx, ctx->synth_cb[i]);
    }
}

void midi_device_read_payload(midi_device_t *ctx, uint8_t b)
{
    // printf("MIDI STATUS: %d\n", ctx->midi_status);
    ...
    if (ctx->payload_index >= ctx->payload_size)
    {
        ...
        midi_device_process_payload(ctx);
        ctx->payload_index = 0;
        ctx->payload_size = 0;
        ctx->state = MIDI_STATE_NEW_STATUS;
...
void midi_device_feed(midi_device_t *ctx, uint8_t b)
{
    ...
    case MIDI_STATE_NEW_STATUS:
        midi_device_process_new_status(ctx, b);
        break;
    case MIDI_STATE_READ_CHANNEL_PAYLOAD:
        midi_device_read_payload(ctx, b);
        break;
    default:
        break;
    }
