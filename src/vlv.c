...
void vlv_set_value(vlv_t* ctx, const uint32_t val)
{
    ...
    {
        ctx->size = (MIDI_VLV_MAX_SIZE - 4);
        return;
    }
    if ((ctx->val & 0x1FC000) != 0)
    {
        ctx->size = (MIDI_VLV_MAX_SIZE - 3);
        return;
    }
    if ((ctx->val & 0x3F80) != 0)
    {
        ctx->size = (MIDI_VLV_MAX_SIZE - 2);
        return;
    }

    ctx->size = (MIDI_VLV_MAX_SIZE - 1);
}

bool vlv_can_fetch(vlv_t* ctx)
{
    return ctx->size <= (MIDI_VLV_MAX_SIZE - 1) && ctx->val != 0;
}
...
uint8_t vlv_fetch(vlv_t* ctx)
{
    ...
    uint8_t masked_val = MIDI_VLV_EXTRACT_BYTE_AT_INDEX(ctx->val, ctx->size);
...
int vlv_encode(vlv_t* ctx, uint8_t* data, const uint32_t size)
{
    ...
    while (vlv_can_fetch(ctx))
    {
        data[iterator++] = vlv_fetch(ctx);
    }
